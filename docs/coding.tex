...
\begin{algorithm}
  \SetKwFunction{greedy}{greedytree}
  \SetKwData{perfect}{perfect}
  \SetKwData{lperfect}{lperfect}
  \SetKwData{rperfect}{rperfect}
  \SetKwFunction{best}{best}
  \SetKwFunction{worst}{worst}
  \SetKwFunction{opttree}{opttree}
  \KwIn{$d$, $N$}
  \KwOut{$(f^{*}(d,N),\perfect)$}
  $(f^{*}_{d,N}, \perfect) \leftarrow \greedy(d,N)$\;
  % $R^{*} \leftarrow R(f^{*}_{d,N})$\;
  \lIf{$\perfect$}{\Return{$(f^{*}_{d,N},\perfect)$}}
  $f_{0} \leftarrow \opttree(d-1,N)$\;
  \ForEach{$j \in P$}{
    $f_{R} \leftarrow f_{0}$,
    $f_{L} \leftarrow f_{0}$\;
    $M_{L} \leftarrow \emptyset$,
    $M_{R} \leftarrow \emptyset$\;
    $R_{L} \leftarrow 0$,
    $R_{R} \leftarrow R(f_{0},N)$\;
    $s'_{L}(N) \leftarrow \emptyset$\;
    \ForEach{$s \in S_{j}$}{
      $M \leftarrow s_{L}(N) \setminus s'_{L}(N)$\;
      $M_{L} \leftarrow M_{L} \cup M$,
      $M_{R} \leftarrow M_{R} \cup M$\;
      \If{$b \leftarrow \best(f_{L},M)$}{
        $R_{L} \leftarrow R(f_{L},s_{L}(N))$\;
        $M_{L} \leftarrow \emptyset$,
        $u_{L} \leftarrow R_{L}$}
      \If{$w \leftarrow \worst(f_{R},M)$}{
        $R_{R} \leftarrow R(f_{R},s_{R}(N))$\;
        $M_{R} \leftarrow \emptyset$,
        $u_{R} \leftarrow R_{R}$}
      \If{$\neg b \vee \neg w$}{
        \lIf{$\neg b$}{$u_{L} \leftarrow R_{L} + \sum_{i \in M_{L}}
          r^{*}(i)$}
        \lIf{$\neg w$}{$u_{R} \leftarrow R_{R} - \sum_{i \in M_{R}}
          r_{*}(i)$}
        \If{$u_{L} + u_{R} \leq R(f^{*}_{d,N},N)$}{
          \lIf{$\neg b$}{$f_{L} \leftarrow
            \mbox{NULL}$}
          \lIf{$\neg w$}{$f_{R} \leftarrow
            \mbox{NULL}$}
      ...
      \If{$\neg b$}{
        $(f_{L},\lperfect) \leftarrow \opttree(d-1,s_{L}(N))$\;
        $R_{L} \leftarrow R(f_{L},s_{L}(N))$\;
        $M_{L} \leftarrow \emptyset$
      }
      \If{$\neg w$}{
        $(f_{R},\rperfect) \leftarrow \opttree(d-1,s_{R}(N))$\;
        $R_{R} \leftarrow R(f_{R},s_{R}(N))$\;
        $M_{R} \leftarrow \emptyset$
      }
      \If{$R_{L} + R_{R} > R(f^{*}_{d,N},N)$}{
        $f^{*}_{d,N} \leftarrow f_{L} + f_{R}$\;
        $\perfect \leftarrow \lperfect \wedge \rperfect$\; 
        \lIf{$\perfect$}{\Return{$(f^{*}_{d,N},\perfect)$}}
      }
      $s'_{L}(N) \leftarrow s_{L}(N)$\;
      }
    }
    \Return{$(f^{*}_{d,N},\perfect)$}
\end{algorithm}
